...
public class ComponenteMapa extends JComponent implements ObservadorSimulacionTrafico {
	...
	public void paint(Graphics graphics) {
		...
		if (this.mapa == null || this.mapa.getListaCruces().size() == 0) {
	...
	private void drawMap(Graphics g) {
		...
		// draw nodes
		for (CruceGenerico<?> j : this.mapa.getListaCruces()) {
		...
		// draw edges
		for (Carretera e : this.mapa.getListaCarreteras()) {
	...
	private void calculaCoordenadasCruce() {
		...
		double slice = 2 * Math.PI / this.mapa.getListaCruces().size();
		int i = 0;
		for (CruceGenerico<?> n : this.mapa.getListaCruces()) {
	...
	@Override
	public void errorSimulacion(int tiempo, MapaCarreteras mapa, List<Evento> eventos, ErrorDeSimulacion e) {
		this.mapa = mapa;
		calculaCoordenadasCruce();
		refresh();
	}
